# scripts/sync_weekly_data_new.py
import pandas as pd
from datetime import datetime, timedelta, date
from sqlalchemy.orm import sessionmaker
from db.database import get_engine_instance
from db.models import StockWeekly, StockList  # <-- 导入 StockWeekly
import time
...
def get_last_weekly_trade_date(symbol: str) -> Optional[date]:
    """从 stock_weekly 表中查找某股票最后的交易周"""
    db_session = Session()
    try:
        # 查询的表从 StockDaily 改为 StockWeekly
        result_proxy = db_session.query(StockWeekly.date).filter_by(symbol=symbol).order_by(
            StockWeekly.date.desc()).first()
        return result_proxy[0] if result_proxy else None
    finally:
        db_session.close()


def fetch_weekly_data(symbol: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
    """调用 WindPy 拉取周线数据"""
    wind_fields = "open,high,low,close,volume,amt,swing,pct_chg,chg,turn"
    try:
        # --- 核心改动：加入 Period='W' 参数 ---
        wind_data = w.wsd(symbol, wind_fields, start_date, end_date, "adj=F;Period=W")
        ...
        print(f"❌ [{symbol}] fetch_weekly_data 函数内部发生异常: {e}")
        return None


# --- 单个股票处理的核心逻辑 ---

def process_single_stock_weekly(stock_info: Tuple[int, pd.Series], today_str_param: str) -> str:
    """处理单个股票的周线数据获取和存储"""
    ...
    # 增量更新模式
    last_date_in_db = get_last_weekly_trade_date(symbol_raw)
    ...
    if start_date_for_api > end_date_for_api:
        return f"⏩ {log_prefix} 周线数据已是最新，无需更新。"

    df = fetch_weekly_data(symbol_for_api, start_date_for_api, end_date_for_api)

    if df is not None and not df.empty:
        try:
            # --- 核心改动：写入 stock_weekly 表 ---
            df.to_sql("stock_weekly", con=engine, index=False, if_exists="append")
            rows_written = len(df)
            time.sleep(SLEEP_PER_TASK)
            return f"✅ {log_prefix} 成功写入 {rows_written} 行周线数据。"
        except Exception as e:
            return f"❌ {log_prefix} 周线数据写入数据库失败: {e}"
    elif df is not None and df.empty:
        return f"ℹ️ {log_prefix} 在指定日期范围未返回周线数据。"
    else:  # df is None
        return f"❌ {log_prefix} fetch_weekly_data 执行失败。"


# --- 主同步函数 (并发版本) ---

def sync_all_weekly_data_concurrent():
    """并发同步所有股票的周线交易数据"""

    # --- 核心逻辑：在同步前，先删除本周已存在的不完整数据 ---
    db_session = Session()
    try:
        today = date.today()
        # 计算本周的星期一 (weekday: Monday is 0 and Sunday is 6)
        start_of_week = today - timedelta(days=today.weekday())

        print(f"🧹 正在删除本周 ({start_of_week.strftime('%Y-%m-%d')} 至今) 已存在的不完整周线数据...")
        stmt = StockWeekly.__table__.delete().where(StockWeekly.date >= start_of_week)
        result = db_session.execute(stmt)
        db_session.commit()
        print(f"✅ 成功删除 {result.rowcount} 条旧的本周记录，准备写入最新数据。")
    except Exception as e:
        db_session.rollback()
        print(f"❌ 删除本周旧数据时发生错误: {e}")
        return
    finally:
        db_session.close()
    # --- 逻辑结束 ---
    ...
    print(f"🚀 开始执行周线增量更新，数据将尝试更新到 {today_str}")

    # (并发执行逻辑与日线脚本相同)
    tasks_args_list = [((idx, row), today_str) for idx, row in stock_df.iterrows()]

    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_WORKERS) as executor:
        futures = [executor.submit(process_single_stock_weekly, args[0], args[1]) for args in tasks_args_list]
        count = 0
...
if __name__ == "__main__":
    print("脚本开始执行：同步周线数据...")
    sync_all_weekly_data_concurrent()
    print("\n周线数据同步脚本执行完毕。")