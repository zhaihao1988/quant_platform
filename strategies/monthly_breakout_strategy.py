# strategies/monthly_breakout_strategy.py
...
try:
    ...
    logger_base_mock = logging.getLogger(__name__ + "_base_mock_monthly_cb")
...
class MonthlyBreakoutStrategy(BaseStrategy):
    def __init__(self, context: StrategyContext):
        ...
        default_params = {
            ...
            'pattern_identification_lookback_months': 24, # 月线级别：约2年
            'pullback_depth_pct': 0.20,
            'days_for_3year_low_months': 36,              # 月线级别：约3年
            'max_price_vs_3year_low_ratio': 2.0,
            'volume_lookback_period': 3,                  # 月线级别：约3个月
    ...
    def strategy_name(self) -> str:
        return "MonthlyChanBreakoutStrategyV1"
    ...
    def run_for_stock(self, symbol: str, current_date: date, data: Dict[str, pd.DataFrame]) -> List[StrategyResult]:
        ...
        monthly_df_orig = data.get("monthly")
        if monthly_df_orig is None or monthly_df_orig.empty or 'date' not in monthly_df_orig.columns:
            logger.warning(f"[{self.strategy_name}@{symbol}@{current_date.isoformat()}] No monthly data.");
            return results

        monthly_df = monthly_df_orig.copy()
        if not pd.api.types.is_datetime64_any_dtype(monthly_df['date']): monthly_df['date'] = pd.to_datetime(
            monthly_df['date'])
        if hasattr(monthly_df['date'].dt, 'date'):
            monthly_df['date'] = monthly_df['date'].dt.date
        else:
            monthly_df['date'] = monthly_df['date'].apply(lambda x: x.date() if isinstance(x, pd.Timestamp) else (
                pd.to_datetime(x).date() if not isinstance(x, date) else x))

        df_full_history_for_analysis = monthly_df[monthly_df['date'] <= current_date].copy()
        ...
        pattern_lookback_months = self.params['pattern_identification_lookback_months']
        df_slice_for_patterns = df_full_history_for_analysis.iloc[
                                -(pattern_lookback_months + 5):].copy()

        if len(df_slice_for_patterns) < (
                self.params['fractal_definition_lookback'] * 2 + self.params['min_bars_between_fractals_bt'] + 3):
            logger.debug(f"[{self.strategy_name}@{symbol}] Not enough data for pattern id."); return results

        raw_klines = self._df_to_raw_klines(df_slice_for_patterns)
        if raw_klines:
            self._merged_klines_state = []; self._current_segment_trend_state = 0
        ...
        lookback_months_for_H = self.params['pattern_identification_lookback_months']
        max_lookback_date = current_date - relativedelta(months=lookback_months_for_H)
        completed_upstrokes = self._get_completed_upstrokes(max_lookback_date=max_lookback_date)

        h1: Optional[H_Point] = None; h2: Optional[H_Point] = None; h3: Optional[H_Point] = None
        ...
        if completed_upstrokes:
            ...
            if h2_candidate_strokes:
                ...
                if h3_candidate_strokes:
                    ...
                    h3 = H_Point(price=h3_stroke.end_fractal.kline.h, date=h3_stroke.end_fractal.kline.dt, level_name="H3")

        if not (h3 and h2 and h1 and h3.price > h2.price > h1.price):
            logger.debug(f"[{self.strategy_name}@{symbol}] No H3>H2>H1 structure. H1:{h1}, H2:{h2}, H3:{h3}"); return results
        logger.debug(f"[{self.strategy_name}@{symbol}] Found: H3({h3.price:.2f}@{h3.date}) > H2({h2.price:.2f}@{h2.date}) > H1({h1.price:.2f}@{h1.date})")
        ...
        for h_target in sorted(potential_breakout_targets, key=lambda p: p.price, reverse=True):
            h_price, h_date, level_name = h_target.price, h_target.date, h_target.level_name
            if h_date >= current_date: continue

            df_after_h = df_full_history_for_analysis[
                (df_full_history_for_analysis['date'] > h_date) & (df_full_history_for_analysis['date'] < current_date)]
            if df_after_h.empty:
                logger.debug(f"[{self.strategy_name}@{symbol}] No data after {level_name} for pullback."); continue
            ...
            if not (min_low_since_h <= required_pullback_price):
                logger.debug(f"[{self.strategy_name}@{symbol}] Pullback for {level_name} not met."); continue

            lookback_months_for_3y_low = self.params['days_for_3year_low_months']
            start_date_3year_low = current_date - relativedelta(months=lookback_months_for_3y_low)
            df_for_3year_low = df_full_history_for_analysis[df_full_history_for_analysis['date'] >= start_date_3year_low]
            ...
            if min_low_past_3_years is not None:
                if current_high_price > min_low_past_3_years * self.params['max_price_vs_3year_low_ratio']:
                    logger.debug(f"[{self.strategy_name}@{symbol}] Price position limit exceeded."); continue
            else:
                logger.warning(f"[{self.strategy_name}@{symbol}] Could not check 3-year low.")

            if not ((previous_high_price < h_price) and (current_high_price > h_price)):
                continue

            if pd.isna(current_ma_volume) or current_ma_volume == 0 or not (current_volume > current_ma_volume * self.params['volume_ratio_threshold']):
                continue

            signal_details = {
                "breakout_level": level_name, "h_price": f"{h_price:.2f}", "h_date": h_date.isoformat(),
                ...
                f"volume_ma{self.params['volume_lookback_period']}": f"{current_ma_volume:.0f}" if pd.notna(current_ma_volume) else "N/A",
            ...
            logger.info(f"[{self.strategy_name}@{symbol}@{current_date.isoformat()}] BUY SIGNAL ({level_name}). Details: {signal_details}")
            return results
        return results
...
if __name__ == "__main__":
    ...
    test_logger = logging.getLogger(__name__ + "_test_main_monthly_cb_v1")
    test_logger.warning("This is a template test block for the MonthlyBreakoutStrategy.")
    test_logger.warning("To run a test, you need to:")
    test_logger.warning("1. Provide a CSV file with monthly stock data (e.g., 'stock_monthly.csv').")
    test_logger.warning("2. Set the 'stock_to_test' symbol and 'date_to_test_str'.")
    # Example:
    # csv_file_path = 'path_to_your_monthly_data.csv'
    # ... etc
